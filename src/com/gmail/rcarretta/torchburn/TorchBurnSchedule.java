...
class TorchBurnSchedule implements Runnable {
    ...
    TorchBurnSchedule(Player newPlayer) {
    ...
    public void run() {
        if (TorchBurn.isLit(player)) {
            if (player.getInventory().getItemInHand().getType() == Material.TORCH) {
                player.getInventory().getItemInHand().setDurability(
                        (short) (player.getInventory().getItemInHand().getDurability() + 1));
                if (player.getInventory().getItemInHand().getDurability() >= 32) {
                    ...
                    TorchBurn.myPlugin.getServer().getScheduler().scheduleSyncDelayedTask(
                            TorchBurn.myPlugin, new TorchBurnSchedule(player), 40);
                }
            }
        }
    }
